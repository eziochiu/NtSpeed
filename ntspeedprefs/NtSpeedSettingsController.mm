...
- (void)twitter
{
	...
	if ([app canOpenURL:[NSURL URLWithString:@"twitter://user?screen_name=ijulioverne"]]) {
		[app openURL:[NSURL URLWithString:@"twitter://user?screen_name=ijulioverne"] options:@{} completionHandler:nil];
	} else if ([app canOpenURL:[NSURL URLWithString:@"tweetbot:///user_profile/ijulioverne"]]) {
		[app openURL:[NSURL URLWithString:@"tweetbot:///user_profile/ijulioverne"] options:@{} completionHandler:nil];
	} else {
		[app openURL:[NSURL URLWithString:@"https://mobile.twitter.com/ijulioverne"] options:@{} completionHandler:nil];
	}
}
- (void)love
{
...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier *)specifier
{
	@autoreleasepool {
		...
		if ([[specifier properties] objectForKey:@"PromptRespring"]) {
            UIAlertController *alertC = [UIAlertController alertControllerWithTitle:self.title message:@"An Respring is Requerid for this option." preferredStyle:(UIAlertControllerStyleAlert)];
            UIAlertAction *alertA = [UIAlertAction actionWithTitle:@"confirm" style:(UIAlertActionStyleDestructive) handler:^(UIAlertAction * _Nonnull action) {
                pid_t pid;
